...
환경값 예시는 [config.properties.example](config.properties.example) 를 참고해서 `./config-[local,dev,prd].properties`로 복사해 사용하면 됩니다. `./config.properties`는 모든 환경에서 공통으로 사용하는 설정을 구성하고 환경별로 오버라이드하여 사용한다.

## 빠른 시작 (로컬)

로컬 실행은 `config-local.properties`에 HF Token, Ray, MinIO, MLflow 값을 설정한 뒤 `CONFIG=local`로 실행하는 방식입니다.
