...
이 플랫폼을 성공적으로 배포하고 실행하기 위해서는 K8s 클러스터가 아래의 최소 시스템 사양을 충족해야 합니다.

- Worker Node 수: 최소 2개 이상
- 노드당 최소 사양:
  - CPU: 4 Core
  - Memory: 12 GB
  - Disk: 50 GB

이 구성에서 배포되는 RayCluster는 기본적으로 CPU 기반으로 Worker Pod를 생성하도록 설정되어 있습니다. 하지만 AI/ML 모델의 학습 성능을 극대화하기 위해서는 GPU 노드 구성이 강력히 권장됩니다

각 서비스는 `platform/` 하위 디렉토리에서 순서대로 설치합니다.


## Step 1. Promtheus, Grafana 설치
```bash
cd platform/prometheus && make crd,apply
cd platform/grafana && make crd, apply
```
...
LLaMA 모델 접근을 위해 https://huggingface.co/meta-llama/Llama-3.2-1B 에서
Meta 라이선스 동의 후 접근 승인을 요청합니다.

`config.properties.example`을 복사해 `config-local.properties`를 만들고,
로컬 환경에 맞게 설정합니다.
...
# 단계별 실행
python -m cli.run_seed        # 데이터 준비
python -m cli.run_analysis    # 데이터 분석
python -m cli.run_validation  # 데이터 검증
python -m cli.run_train       # 학습
python -m cli.run_eval        # 평가
python -m cli.run_promote     # 모델 등록
```
...
## Part 3. Airflow 파이프라인

### 3-1 LLaMA 학습 파이프라인 사전 준비
...
kubectl -n mlops create secret generic hf-secret \
...
| `dag_taxi_xgboost_pipeline.py` | `platform/airflow/dags/pipeline/` | NYC Taxi 요금 예측 |
