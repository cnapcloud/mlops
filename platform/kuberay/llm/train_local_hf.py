...
python train_hf.py --device mps
python train_hf.py --device cuda
python train_hf.py --device cpu

# train 모드 + device 지정
python train_hf.py --mode train --device mps
python train_hf.py --mode train --device cuda
python train_hf.py --mode train --device cpu
"""
...
def _training_args_for_device(device: torch.device, output_dir: str) -> TrainingArguments:
    ...
    return TrainingArguments(
        ...
        use_cpu=is_cpu,
        no_cuda=not is_cuda,          # MPS 사용 시 CUDA 비활성화 명시
...
def build_dataset(tokenizer):
    ...
    def tokenize(example):
        ...
        tokens["labels"] = [
            ...
            for label, mask in zip(tokens["input_ids"], tokens["attention_mask"])
        ]
        return tokens
...
def inference(device: torch.device):
    ...
    if tokenizer.eos_token_id in generated:
        eos_idx = (generated == tokenizer.eos_token_id).nonzero(as_tuple=True)[0][0].item()
