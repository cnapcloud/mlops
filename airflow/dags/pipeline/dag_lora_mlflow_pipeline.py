...
TEMPLATE_PATH = os.path.join(DAG_FOLDER, 'manifests', 'lora_pipeline_template.yaml')
...
subprocess.run([sys.executable, '-m', 'pip', 'install', 'mlflow==3.12.0', '-q'], check=True)
import os
import json
import tempfile
import warnings
...
print("5. [등록] 검증이 통과된 LoRA 어댑터를 MLflow에 업로드합니다.")

mlflow.set_tracking_uri("http://mlflow.mlops.svc.cluster.local")

client = MlflowClient()
experiment = client.get_experiment_by_name("lora-finetune")
if experiment:
    for r in client.search_runs(
        experiment_ids=[experiment.experiment_id],
        filter_string="attributes.status = 'RUNNING'",
    ):
        client.set_terminated(r.info.run_id)

mlflow.set_experiment("lora-finetune")

with mlflow.start_run(run_name="lora-registration") as run:
    mlflow.log_params({
    ...
    mlflow.log_metric("eval_accuracy", 0.95)

    with tempfile.TemporaryDirectory() as tmpdir:
        dummy_model_path = os.path.join(tmpdir, "adapter_model.bin")
        ...
        mlflow.pyfunc.log_model(
            ...
            artifacts={
                "adapter_model": dummy_model_path,
                "adapter_config": config_path,
            },
            registered_model_name="llama3-lora",
        )

    versions = client.get_latest_versions("llama3-lora")
    latest = versions[-1]
    client.set_registered_model_alias("llama3-lora", "staging", latest.version)

    output = {
        "run_id": run.info.run_id,
        "model_name": "llama3-lora",
        "model_version": latest.version,
        "model_uri": "models:/llama3-lora@staging",
...
with DAG(
    dag_id='llm_lora_mlflow_pipeline',
    ...
    model_registration = KubernetesPodOperator(
        ...
        env_vars=[
            V1EnvVar(name="MLFLOW_TRACKING_USERNAME", value="admin"),
            V1EnvVar(name="MLFLOW_TRACKING_PASSWORD", value="password1234"),
        ],        
        pod_template_file=TEMPLATE_PATH,
        cmds=['python', '-c'],
        arguments=[MODEL_REGISTRATION_SCRIPT],
        do_xcom_push=True,
        on_finish_action="delete_succeeded_pod",
        get_logs=True,
    )
